...
public class JargonMetadataResolverTest {
	...
	public void testSaveMetadataTemplateAsJson() throws Exception {
		...
		MetadataTemplate metadataTemplates = resolver.listAllTemplatesBoundToPath(targetIrodsCollection);
	...
	public void listPublicTemplatesNoDuplicates() throws Exception {
		...
		dataTransferOperations.putOperation(TEMPLATE_FILE_NAME2, targetIrodsCollection1,
				irodsAccount.getDefaultStorageResource(), null, null);
		...
		MetadataTemplateConfiguration metadataConfiguration = new MetadataTemplateConfiguration();
		metadataConfiguration.setPublicTemplateIdentifier(testDirName1);
	...
	public void listAllTemplatesNoDuplicates() throws Exception {
		...
		MetadataTemplateConfiguration metadataConfiguration = new MetadataTemplateConfiguration();
		metadataConfiguration.setPublicTemplateIdentifier(targetIrodsCollection3);
		...
		metadataTemplates = resolver.listAllTemplatesBoundToPath(targetIrodsCollection2);
	...
	public void findTemplateByNameInPublicTemplatesFilePresent() throws Exception {
		...
		MetadataTemplateConfiguration metadataConfiguration = new MetadataTemplateConfiguration();
		metadataConfiguration.setPublicTemplateIdentifier(targetIrodsCollection1);
		JargonMetadataResolver resolver = new JargonMetadataResolver(context, accessObjectFactory,
				metadataConfiguration);
	...
	public void testFindByUUID() throws Exception {
		...
		MetadataTemplate mdTemplate = resolver.findTemplateByUUID(uuid.toString());
		Assert.assertNotNull("no template found", mdTemplate);
	}
	...
	public void saveFormBasedTemplateAsJsonGivenMdTemplateDir() throws Exception {
		...
		String pathToSavedFile = resolver.saveTemplate(metadataTemplate, mdTemplatePath2);
	...
	public void saveFormBasedTemplateAsJsonGivenParentDir() throws Exception {
		...
		String pathToSavedFile = resolver.saveTemplate(metadataTemplate, targetIrodsCollection2);
	...
	public void updateFormBasedTemplateValid() throws Exception {
		...
		String mdTemplateFqName2 = resolver.saveTemplate(template, targetIrodsCollection2);
		...
		boolean retVal = resolver.updateTemplate(mdTemplateFqName2, template);
	...
	public void updateFormBasedTemplateFqNameInvalid() throws Exception {
		...
		String mdTemplateFqName3 = resolver.saveTemplate(template, targetIrodsCollection1);

		template = resolver.findTemplateByFqName(mdTemplateFqName2);

		String mdTemplateFqName4 = resolver.saveTemplate(template, targetIrodsCollection2);
		...
		boolean retVal = resolver.updateTemplate(mdTemplateFqName4, template);
	...
	public void deleteTemplateByFqNameValid() throws Exception {
		...
		boolean retVal = resolver.deleteTemplate(mdTemplateFqName);
	...
	public void deleteTemplateByFqNameNotATemplateFile() throws Exception {
		...
		resolver.deleteTemplate(mdTemplateFqName);
	}
	...
	public void getAndMergeTemplateWithRefIrodsQueryForFile() throws Exception {
		...
		for (MetadataElement me : template.getElements()) {
			if (me.getName().equalsIgnoreCase("data_name")) {
				Assert.assertEquals("data.name not in currentValue", "data.name", me.getCurrentValue().get(0));
				Assert.assertEquals("data.name not populated", TEST_FILE_NOPATH, me.getDisplayValue().get(0));
			} else if (me.getName().equalsIgnoreCase("data_owner_name")) {
				Assert.assertEquals("data.owner_name not in currentValue", "data.owner_name",
						me.getCurrentValue().get(0));
				Assert.assertEquals("data.owner_name not populated",
						testingPropertiesHelper.getTestProperties().getProperty(TestingPropertiesHelper.IRODS_USER_KEY),
						me.getDisplayValue().get(0));
			} else if (me.getName().equalsIgnoreCase("data_owner_zone")) {
				Assert.assertEquals("data.owner_zone not in currentValue", "data.owner_zone",
						me.getCurrentValue().get(0));
				Assert.assertEquals("data.owner_zone not populated",
						testingPropertiesHelper.getTestProperties().getProperty(TestingPropertiesHelper.IRODS_ZONE_KEY),
						me.getDisplayValue().get(0));
			} else if (me.getName().equalsIgnoreCase("user_type")) {
				Assert.assertEquals("user.type not in currentValue", "user.type", me.getCurrentValue().get(0));
				Assert.assertEquals("user.type not populated", "rodsuser", me.getDisplayValue().get(0));
			}
		}
	}
	...
	public void getAndMergeTemplateWithRefIrodsQueryForCollection() throws Exception {
		...
		for (MetadataElement me : template.getElements()) {
			if (me.getName().equalsIgnoreCase("coll_owner")) {
				Assert.assertEquals("coll.owner not in currentValue", "coll.owner", me.getCurrentValue().get(0));
				Assert.assertEquals("coll.owner not populated",
						testingPropertiesHelper.getTestProperties().getProperty(TestingPropertiesHelper.IRODS_USER_KEY),
						me.getDisplayValue().get(0));
			} else if (me.getName().equalsIgnoreCase("coll_owner_zone")) {
				Assert.assertEquals("coll.owner_zone not in currentValue", "coll.owner_zone",
						me.getCurrentValue().get(0));
				Assert.assertEquals("coll.owner_zone not populated",
						testingPropertiesHelper.getTestProperties().getProperty(TestingPropertiesHelper.IRODS_ZONE_KEY),
						me.getDisplayValue().get(0));
			}
		}
